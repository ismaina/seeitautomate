...
class SeeItAtutomate:
    ...
    def NetmAuthenticate(self,nt_user,nt_pw,nt_IP_Address,nt_Vendor):
        ...
        f = open("port_density_results.txt", "a")
        if my_device['device_type'] == 'cisco_ios': #Execute Huawei Commands
            ...
            try:
                ...
                if nt_pw: #if authenticated then push commands
                    ...
                    output = login.send_command("show interface brief | i ^Fast.*up.*up|^Gig.*up.*up")
                    ...
                    print('>>>>>>>>>__Cisco Authentication failure??<<<<<<<<')  

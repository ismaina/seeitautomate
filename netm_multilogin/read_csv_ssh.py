...
class SeeItAtutomate:
        ...
        with open('Enterprise_Access_Switch_Descriptions.csv') as ip_add_f: # a simple list of IP addresses you want to connect to each one on a new line 
            ...
            for row in reader:
                ...
                try:   #Attempt to login to device and put commands
                    ...
                    multi_p = mp.Process(target=self.NetmAuthenticate(cred_uname,cred_pw,self.IP_Address,self.Vendor,self.mp_queue),args = (cred_uname,cred_pw,self.IP_Address,self.Vendor,self.mp_queue))
    ...
    def NetmAuthenticate(self,nt_user,nt_pw,nt_IP_Address,nt_Vendor,mp_queue):
